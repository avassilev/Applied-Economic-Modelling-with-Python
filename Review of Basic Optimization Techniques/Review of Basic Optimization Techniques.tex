...
\begin{section}{Static Optimization with Equality Constraints. Lagrange Multipliers}\label{sec:Lagr}
	...
	\begin{frame}[fragile]
		...
		where $ \boldsymbol{\lambda }= (\lambda_1,\ldots,\lambda_m)' $ is the vector of Lagrange multipliers.\bigskip
	...
	\begin{frame}[fragile]
		...
		\begin{block}{Algorithm}
			\begin{enumerate}
				...
				\item Set the resulting derivatives equal to zero, i.e. construct the first-order conditions
				\begin{equation}
	...
	\end{frame}
		...
		\begin{Fact}
			...
			the Lagrangian $\mathcal{L}(x)$ is concave (convex) in $x$, and
