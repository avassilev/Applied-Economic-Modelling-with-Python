...
\end{defn}
\begin{itemize}
	\item Note that the unknown object in a differential equation is a \textit{function}, not a \textit{number}
...
\begin{defn}
	...
	\]
	i.e. the unknown function $x(t)$ and its derivatives enter the equation linearly and the coefficients $a_i(t)$ and the right-hand side 
	$g(t)$ may depend on time $t$ (but not on $x$).
\end{defn}
\begin{itemize}
	\item A non-linear ODE is one not fitting the above definition
	\item Example of a non-linear ODE:
	\[
...
\begin{defn}
	\textit{Autonomous} ODE are equations in which time is not explicitly present as an independent variable and hence can be written in the form
	\[ \dot{x} = F(x). \]	
\end{defn}
\begin{itemize}
	...
	\item Non-autonomous equations take the form $\dot{x} = F(t,x)$ 
