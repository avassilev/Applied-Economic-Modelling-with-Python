...
\begin{section}{The maximum principle}\label{sec:max}
	...
	\begin{frame}[fragile]
		...
		\begin{itemize}\itemsep1em
			...
			\item Here we'll follow the first one as it is generally more tractable and is widely used in economic applications.
	...
	\begin{frame}[fragile]
		...
		\begin{itemize}\itemsep1em
			...
			\item When it is unclear whether the maximum is attained at an interior point, a more general approach to solving the Hamiltonian maximization problem is called for.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}[fragile]
		\frametitle{The maximum principle in action}\addtocounter{exmpl}{-1}
		\begin{exmpl}[cont.]
	...
	\begin{frame}[fragile]
		...
		\begin{exmpl}[cont.]
			...
			Since $ t\in [0,1] $, it follows that $ \lambda(t)\leq 0 $. More precisely, this implies that $ u^*(t)=-1 $ for $ t\in[0,1) $. As $ u^* $ is undetermined for $ \lambda = 0 $, we can choose to define it to be $ -1 $ when $ t=1 $.
		\end{exmpl}
	\end{frame}
	...
	\begin{frame}[fragile]
		...
		\begin{exmpl}[cont.]
			...
			\textbf{Note:} It can be shown using alternative arguments that the optimal $ c(t) $ and $ w(T) $ are positive, so the logarithms are well-defined and we can apply the maximum principle. Here we take it for granted.\bigskip
...
\begin{section}{Miscellaneous info on optimal control problems and the maximum principle}\label{sec:misc}
	...
	\begin{frame}[fragile]
		\frametitle{Sufficiency}
		\begin{exmpl}[Example \ref{ex:simpleOC} revisited]
		...
		\begin{exmpl}[Example \ref{ex:bangbang} revisited]
